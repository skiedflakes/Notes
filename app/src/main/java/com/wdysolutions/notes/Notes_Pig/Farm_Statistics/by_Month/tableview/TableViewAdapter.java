...
package com.wdysolutions.notes.Notes_Pig.Farm_Statistics.by_Month.tableview;
...
import com.wdysolutions.notes.Notes_Pig.Farm_Statistics.by_Month.tableview.holder.CellViewHolder;
import com.wdysolutions.notes.Notes_Pig.Farm_Statistics.by_Month.tableview.holder.ColumnHeaderViewHolder;
import com.wdysolutions.notes.Notes_Pig.Farm_Statistics.by_Month.tableview.holder.RowHeaderViewHolder;
...
public class TableViewAdapter extends AbstractTableAdapter<ColumnHeader, RowHeader_byMonth, Cell> {
        ...
        if (cell.getPercent_bg().equals("red") && cell.getPercent_underline().equals("yes")){
            ...
            cellViewHolder.percent.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    openDialogTable(String.valueOf(columnPosition+1),
                            Constants.rowHeaders_bymonth.get(rowPosition).getFarm_statistics_unique(),
                            Constants.selectedYear, "month");
                }
            });
    ...
    public void onBindRowHeaderViewHolder(AbstractViewHolder holder, Object rowHeaderItemModel, int rowPosition) {
        final RowHeader_byMonth rowHeaderByMonth = (RowHeader_byMonth) rowHeaderItemModel;
        final RowHeaderViewHolder rowHeaderViewHolder = (RowHeaderViewHolder)holder;

        rowHeaderViewHolder.row_header_textview.setText(rowHeaderByMonth.getCurrent());
        rowHeaderViewHolder.row_header_textview_ave.setText(rowHeaderByMonth.getAve());
        rowHeaderViewHolder.row_header_textview_percent.setText(rowHeaderByMonth.getPercent());
        rowHeaderViewHolder.row_header_textview_name.setText(rowHeaderByMonth.getName());

        if (rowHeaderByMonth.getCounter_clickable() > 0){
        ...
        rowHeaderViewHolder.row_header_textview_name.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                openMenuGraph(rowHeaderViewHolder, rowHeaderByMonth, "bymonth");
            }
        });
    }
    ...
    private void openMenuGraph(RowHeaderViewHolder holder, final RowHeader_byMonth rowHeaderByMonth, final String table_selected){
        PopupMenu popup = new PopupMenu(mContext, holder.row_header_textview_name);
        popup.inflate(R.menu.menu_table);

        if (rowHeaderByMonth.getDropdown_current_graph().equals("1")){ popup.getMenu().getItem(0).setVisible(true); }
        if (rowHeaderByMonth.getDropdown_ave_graph().equals("1")){ popup.getMenu().getItem(1).setVisible(true); }
        if (rowHeaderByMonth.getDropdown_perc_graph().equals("1")){ popup.getMenu().getItem(2).setVisible(true); }
        popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                ...
                if (item.getItemId() == R.id.action_curr){ // current
                    selected = rowHeaderByMonth.getFarm_statistics_curr();
                } else if (item.getItemId() == R.id.action_ave){ // average
                    selected = rowHeaderByMonth.getFarm_statistics_ave();
                } else { // percentage
                    selected = rowHeaderByMonth.getFarm_statistics_perc();
                }
    ...
    private void openDialogTable(String month, String module, String year, String table_selected){
        Bundle bundle = new Bundle();
        bundle.putString("table_selected", table_selected);
        bundle.putString("week_month", month);
